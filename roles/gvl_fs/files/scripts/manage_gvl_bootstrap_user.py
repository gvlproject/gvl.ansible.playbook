...
class GVLBootstrapGalaxyApplication( object ):
    """
...
def create_user( app, email, password, username ):
    if email and password and username:
        ...
        log.error("Missing required values for email: {0}, password: {1}, username: {2}".format(email, password, username))
    return None
...
def get_bootstrap_app(ini_file):
    ...
    app = GVLBootstrapGalaxyApplication( config )
    return app

def create_gvl_bootstrap_user(ini_file, username, user_email, password):
    ...
    if user is not None:
        ...
        log.error("Problem creating a new user: {0} and an associated API key.".format(username))
        exit(1)

def delete_gvl_bootstrap_user(ini_file, username):
    app = get_bootstrap_app(ini_file)
...
if __name__ == "__main__":
    ...
    parser_create.add_argument("-u", "--username",
                        default="buildbot",
                        help="Username to create. Defaults to buildbot",)
    parser_create.add_argument("-e", "--email",
                        default="buildbot@genome.edu.au",
                        help="Email for user",)
    ...
    parser_delete.add_argument("-u", "--username",
                        default="buildbot",
                        help="Username to delete. Defaults to buildbot",)
    args = parser.parse_args()

    if args.action == "create":
        create_gvl_bootstrap_user(args.config, args.username, args.email, args.password)
    elif args.action == "delete":
        delete_gvl_bootstrap_user(args.config, args.username)
